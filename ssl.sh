...
install_dependencies() {
    ...
    if [[ "$OS_TYPE" == "ubuntu" || "$OS_TYPE" == "debian" ]]; then
        dependencies=("curl" "socat" "cron") # 移除 ufw
    elif [[ "$OS_TYPE" == "centos" || "$OS_TYPE" == "rhel" ]]; then
        dependencies=("curl" "socat" "cronie") # 移除 firewalld
    else
...
issue_cert() {
    ...
    # --pre-hook 和 --post-hook 用于停止/启动 Web 服务器，以释放 80 端口，如果您的 Web 服务不在运行，可以移除它们。
...
install_cert() {
    ...
    # !!! 注意 reloadcmd 需要根据 Web 服务器修改，此处保留默认的 Nginx/Apache
...
echo -e "${YELLOW}重要提示: 由于移除了防火墙配置，请确保服务器的 80 端口（用于 Let's Encrypt 验证）以及您自己的服务端口是开放的。${RESET}" >&2
